...
当 Agent 有 sub-agent 能力（如 OpenClaw sessions_spawn）时，**优先使用双线并行**：
...
│    ├─ sysctl 调整?             系统诊断              │
│    └─ cgroup 限制?             (journalctl/systemctl)│
│                                                      │
│  两者同时 fork 执行，互不等待                          │
└─────────────────────────────────────────────────────┘
    │
...
│  输出两条轨道的方案：                                │
│  {                                                   │
│    "track_a_nonmodifying": { "systemd": ..., ... },  │
│    "track_b_modifying": { "fetch": ..., "info": ... }│
│  }                                                   │
│                                                      │
│  ▶ Agent 分析两条方案的优缺点                        │
│  ▶ 向用户展示对比，等待决策                          │
...
│                                                      │
│  选 Track A（不修改 OS）:                             │
│    → systemd drop-in / sysctl / cgroup 配置          │
│    → 无需 utop build，直接应用配置                    │
│                                                      │
│  选 Track B（修改 OS）:                               │
│    → utop patch create → build → verify → install    │
│    → 进入标准构建流程                                 │
└─────────────────────────────────────────────────────┘
```

**什么时候用双线并行：**
- Agent 有 sub-agent 能力（可以同时跑多个 utop 子命令）
- 问题有"不修改 OS"和"修改 OS"两种解决路径
- 用户没有明确指定方案

**什么时候用标准单线：**
- 用户明确说"我要 patch 这个包"（直接走 Track B）
- 用户明确说"我不想改系统"（直接走 Track A）
- 内核级修改（直接走 kpatch 工作流）
- 没有 sub-agent 能力
